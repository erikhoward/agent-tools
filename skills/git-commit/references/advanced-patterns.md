...
**As a `commit-msg` hook** (via Husky v9+):
...
(The `husky add` subcommand was removed in husky v9 — write the hook file directly.)
...
standard-version is archived and no longer maintained. New projects should use semantic-release or [release-please](https://github.com/googleapis/release-please). Existing projects still on it:
...
- [ ] Description does not start with a capital letter (convention, not spec)
- [ ] Description does not end with a period (convention, not spec)

> Items marked "convention, not spec" follow the Angular style guide adopted by this skill; the Conventional Commits 1.0.0 spec permits any casing.

**Body**
- [ ] If body is present, there is exactly one blank line between description and body
- [ ] Body wraps at 72 characters per line (convention, not spec)
