...
- [Commit Types — Detailed Reference](#commit-types--detailed-reference)
  ...
  - [chore](#chore)
  - [revert](#revert)
  - [Overlapping Types — Decision Guide](#overlapping-types--decision-guide)
    - [`fix` vs `refactor`](#fix-vs-refactor)
    - [`feat` vs `refactor`](#feat-vs-refactor)
    - [`build` vs `chore`](#build-vs-chore)
    - [`style` vs `refactor`](#style-vs-refactor)
    - [When a commit touches multiple types](#when-a-commit-touches-multiple-types)
...
**SemVer impact**: varies (mirrors the reverted commit; semantic-release maps `revert` to PATCH by default)
