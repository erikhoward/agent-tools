...
golangci-lint is a fast, parallel Go linters aggregator. It runs dozens of linters concurrently, caches results, and reports issues in a unified output. Use this skill to install, configure, run, and integrate golangci-lint into Go projects. For Go writing conventions, use the `go` skill; this skill enforces them mechanically (see the Style Enforcement Map below).

## Installation

Pin the version across your team; check https://github.com/golangci/golangci-lint/releases for the latest tag (examples here use `v2.12`).
...
```bash
curl -sSfL https://golangci-lint.run/install.sh | sh -s -- -b $(go env GOPATH)/bin v2.12
...
```bash
curl -sSfL https://golangci-lint.run/install.sh | sh -s v2.12
```
...
```bash
docker run --rm -t --user $(id -u):$(id -g) -v $(pwd):/app -w /app golangci/golangci-lint:v2.12 golangci-lint run
```

`--user` prevents the container from chowning mounted files to root; see `references/advanced-configuration.md` for a cache-volume version.
...
If a project has a v1 config (top-level `linters-settings`, `issues.exclude-rules`, `run.skip-files`), rewrite it automatically:

```bash
golangci-lint migrate
```

This rewrites the config in place — commit or back it up first. Common v1→v2 moves: `linters-settings` → `linters.settings`, `issues.exclude-rules` → `linters.exclusions.rules`, and formatters (`gofmt`, `goimports`, etc.) move out of `linters.enable` into their own `formatters:` section.
...
In v2, formatters (`gofmt`, `gofumpt`, `goimports`, `gci`, `golines`, `swaggo`) are configured separately from linters. Listing them under `linters.enable` fails config validation:
...
Apply enabled formatters with `golangci-lint fmt`.

## Style Enforcement Map

The `go` skill defines writing conventions (Google Go Style Guide); this skill enforces them. Write with the `go` skill, enforce with golangci-lint:

| Convention (`go` skill) | Enforced by |
|---|---|
| `gofmt` formatting | `formatters.gofmt`, `golangci-lint fmt` |
| Import grouping and ordering | `formatters.goimports`, `gci` |
| Naming conventions and initialisms | `revive` (`var-naming`) |
| Import alias rules | `importas` |
| Unchecked errors | `errcheck` |
| Error wrapping with `%w` | `errorlint` |
| Wrapping errors from external packages | `wrapcheck` |
| Doc comments on exported items | `revive` (`exported`) |
| Comment punctuation | `godot` |
| Function length | `funlen` |
| Cyclomatic complexity | `gocyclo`, `cyclop` |
| Unused code | `unused` |
| Bugs and suspicious constructs | `govet`, `staticcheck`, `gocritic` |
  ...
  lint:
    ...
    steps:
      ...
      - uses: actions/setup-go@v5
        with:
          go-version: stable
      - uses: golangci/golangci-lint-action@v7  # v7+ required for golangci-lint v2
        with:
          version: v2.12
          args: --timeout=5m
```

Use the official `golangci/golangci-lint-action` — it handles caching and creates GitHub annotations for found issues. Action v7 or later is required for golangci-lint v2 (`@v6` rejects v2 version strings), and `actions/setup-go` must run before it.

### Other CI (Binary)

```bash
curl -sSfL https://golangci-lint.run/install.sh | sh -s -- -b /usr/local/bin v2.12
golangci-lint run ./...
```
