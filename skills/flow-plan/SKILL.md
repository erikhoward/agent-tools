...
- **Read the concept brief first.** If `.opencode/concepts/<feature-name>.md`
  exists (from `/flow-ideate`), read it in full before asking anything. Its
  What It Is, Key Decisions, Known Constraints, and Open Risks are
  pre-answered requirements — carry them into the plan verbatim. Only ask the
  user about the brief's Open Questions, never about what it has already
  decided. If no brief exists, proceed normally.
...
- **Read the concept brief if it exists.** `.opencode/concepts/<feature-name>.md` pre-answers requirements; don't re-ask what ideation already decided.
...
Ask for feature name (kebab-case)
    │
    ▼
Concept brief exists? → read it (pre-answered requirements)
    │
    ▼
