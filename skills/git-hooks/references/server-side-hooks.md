...
while read old_sha new_sha ref_name; do
    ...
    # Validate each commit message (first line only)
    for commit in $(git rev-list "$range"); do
        message=$(git log -1 --pretty=%B "$commit")
        COMMIT_MSG=$(echo "$message" | head -n1)
        
        # Canonical Conventional Commits pattern, kept in sync with the parent SKILL.md / git-commit skill
...
All commits must follow Conventional Commits format:
  type(scope)!: description (subject max 72 chars, first line only)
...
while read old_sha new_sha ref_name; do
    ...
    for commit in $(git rev-list "$range"); do
        ...
        for file_path in $(git diff-tree --no-commit-id --name-only -r "$commit"); do
            # Note: `git cat-file -s` returns 0 (or fails) for deleted/renamed-away
            # paths, so skip files that don't exist at this commit
            git cat-file -e "$commit:$file_path" 2>/dev/null || continue
            
            file_size=$(git cat-file -s "$commit:$file_path")
            
            if [ "$file_size" -gt "$MAX_SIZE" ]; then
                size_mb=$((file_size / 1024 / 1024))
                echo "❌ Push rejected: File '$file_path' is ${size_mb}MB (max 10MB)"
                echo "Commit: $commit"
                echo ""
...
echo "$(git rev-parse HEAD~1) $(git rev-parse HEAD) refs/heads/main" | .git/hooks/pre-receive
```
