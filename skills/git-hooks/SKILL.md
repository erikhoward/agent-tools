...
# Type list kept in sync with the git-commit skill (Conventional Commits 1.0.0)
...
Validate only the **first line** (`head -n1`) so a conforming body line can't mask an invalid subject. The regex accepts optional scopes and the `!` breaking-change marker, matching the `git-commit` skill.
...
## Production Hook System Layout

A complete hooks system for a project looks like this.
...
Team members run once after cloning:
```bash
git config core.hooksPath .githooks
```

Optionally ship a `setup-hooks.sh` that runs this and verifies hook permissions.

### Hook Naming Convention

Name modular hooks with a numeric prefix:
```
<number>-<description>.sh
```
...
- **`references/hook-types.md`** - Git hooks reference with parameters and use cases
- **`references/server-side-hooks.md`** - Server-side hooks for policy enforcement
...
For commit-message validation rules, defer to the `git-commit` skill (Conventional Commits 1.0.0) and keep hook regexes in sync with it.

### Example Implementations

Working examples in this skill:
