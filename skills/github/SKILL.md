...
> Commands in this skill were verified against gh 2.97. The core command surface is stable across minor versions. If a flag seems missing, check `gh --version` and the command's `--help`.
...
The agentic baseline (`NO_COLOR=1`, `GH_NO_UPDATE_NOTIFIER=1`, `GH_PAGER=cat`) is documented under [Agentic Output Principles](#agentic-output-principles) — set it before any gh command.
...
## Beyond the CLI: gh api Surfaces

Some GitHub features have no dedicated `gh` subcommand — use `gh api`:
...
| Branch protection rules | `gh api repos/{owner}/{repo}/branches/main/protection` |
| Rulesets (mutations) | `gh ruleset` is **read-only**; create/edit via `gh api repos/{owner}/{repo}/rulesets` |
| Rate limit status | `gh api rate_limit --jq '.resources.core'` |

GraphQL queries and mutations both go through `gh api graphql -f query='...'`. When iterating with cursors, use `--paginate` (never prompt for a missing `$endCursor` variable — gh will hang waiting for input).
...
- **Extending gh:** `gh extension browse` / `gh extension install <repo>` add community commands (e.g. `gh-merge`).
