...
gh search commits "author-date:2026-01-01..2026-04-01" --sort author-date
# or: committer-date:2026-01-01..2026-04-01 with --sort committer-date
# (path:/after:/before: are not commit-search qualifiers; path: is code search only)
```
...
```bash
# Custom headers (preview media types are resource-specific; see each
# endpoint's docs for its Accept value)
gh api repos/{owner}/{repo}/issues \
...
gh search issues "is:open" --json number,title,author,createdAt \
  --jq '.[] | [.number, .title, .author.login, .createdAt] | @csv'
```

### Bulk-close issues with a label

See the bulk-edit patterns in `issue-workflows.md` (list by label, close via `xargs`).
