...
> Bare `gh pr merge 42` prompts interactively, which hangs scripts and agents.
> Always pass an explicit method: `--merge`, `--squash`, or `--rebase`
> (optionally with `--delete-branch` and/or `--auto`).
...
```bash
gh pr edit 42 --add-label bug --add-label urgent
gh pr edit 42 --remove-label bug
gh pr edit 42 --add-assignee alice
gh pr edit 42 --remove-assignee alice
gh pr edit 42 --add-reviewer bob,@team/reviewers
gh pr edit 42 --remove-reviewer bob
...
```bash
gh pr edit 42 --add-label new-label
```
...
```bash
# Watches PR checks until complete; exits non-zero on failure
gh pr checks 42 --watch
```
