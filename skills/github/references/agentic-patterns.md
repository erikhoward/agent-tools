...
Inefficient: `gh pr list --json number,title,author,labels,assignees,reviewDecision,statusCheckRollup` when only `number` is needed
Efficient: `gh pr list --json number` → minimal output per PR
...
```bash
export NO_COLOR=1               # Strip ANSI escape codes (\033[...)
export GH_NO_UPDATE_NOTIFIER=1  # Suppress "gh has a new version" banner
export GH_PAGER=cat             # Disable less paging prompts
```

**Impact:** removes banner, pager, and escape-code noise from every command.
...
```bash
export NO_COLOR=1 GH_NO_UPDATE_NOTIFIER=1 GH_PAGER=cat
```
...
### Requesting Too Many Fields (Avoid)

**When NOT to use:**
- Requesting more fields than needed, e.g. `gh pr list --json number,title,author,labels,assignees,reviewDecision,statusCheckRollup` when only the number is needed
- `gh api` without field projection → returns all fields of all objects
- Any command where you're not using most returned fields

**Impact:** 3-5x more tokens than the minimal-field equivalent

(Bare `gh pr list --json` with no fields is not verbose output — it exits 1 and prints the available-field list. Use it to discover fields, then request only what you need.)

```bash
# BAD: nested author/labels/statusCheckRollup objects when only the number is needed
gh pr list --json number,title,author,labels,assignees,reviewDecision,statusCheckRollup
...
| CI status | `gh pr checks 42 --json state --jq '.[] \| .state'` | 1 word per check |
...
| Run jobs | `gh run view 12345 --json jobs --jq '.jobs[].name'` | ~30 chars per job |
...
# GOOD: file names only
gh pr diff 42 --name-only

# GOOD: check for a specific file
gh pr diff 42 --name-only | grep parser
```
...
See **references/search-and-query.md → Pagination** for the bulk GraphQL fetch pattern.
...
```bash
# Don't: gh pr list --json number,title,state,body (then filter open PRs in context)
# Do:
...
```bash
# Don't: gh issue list --json number,title,body (then restructure in context)
# Do:
...
```bash
# Don't: gh issue list --json number,title,body (then count in context)
# Do:
...
|---|---|---|
| Requesting more fields than needed, e.g. `gh pr list --json number,title,author,labels,assignees,reviewDecision,statusCheckRollup` for a count | Nested author/labels/statusCheckRollup objects bloat output | Use `--json number` |
...
```bash
gh pr list --json number,title,author,labels,statusCheckRollup
# ... agent reads every author object, label, and status check, picks out mergeable ones
```
