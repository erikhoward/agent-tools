...
```bash
# Clone by owner/repo, then cd into it
...
# Create autolink (example: JIRA integration)
gh repo autolink create JIRA "https://jira.company.com/browse/JIRA-<num>"
...
Verify build provenance (attestations) for release artifacts:

```bash
# Download a release asset, then verify its attestation
gh release download v1.0.0 --pattern "app.zip"
gh attestation verify app.zip

# Verify in the context of a specific repository
gh attestation verify app.zip -R owner/repo
```
